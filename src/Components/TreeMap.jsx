...
function TreeMap(props) {
  ...
  var value = <li key={Math.floor(Math.random() * 1000)} onClick={() => { props.changeState(false); props.updateTree(props.tree)}}>{props.tree.value}</li>;
  var children;
    ...
    children = (
      <ul>
        {props.tree.children.map((branch, index) => {
          return <TreeMap changeState={props.changeState} updateTree={props.updateTree} key={index} tree={branch}/>
        })}
      </ul>
    );
  }
